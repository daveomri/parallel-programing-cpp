...
class Graph {
    ...
    //--------------------------------------
        Graph(int numNodes): nodesNum(numNodes), weightsSum(0), edgesNum(0), edges(NULL) {
            int matrixSize = numNodes*(numNodes-1)/2;
        ...
        int getMatrixPosition(int x, int y) {
            int gre = x > y ? x : y;
            int low = gre == x ? y : x;
            ...
            //int i = 0;
            int sum = ((low) * ((this->nodesNum - 1) + (this->nodesNum - low)) / 2) + (gre - low - 1);
            //std::cout << "gre: " << gre << " low: " << low << " sum: " << sum << '\n';
            // std::cin >> i;
        ...
        void setEdgeWeight(int x, int y, int weight) {
            ...
            if (x == y) return;
            matrix[this->getMatrixPosition(x, y)] = weight;
        }
        ...
        int getEdgeWeight(int x, int y) {
            ...
            if (x == y) return 0;
            return matrix[this->getMatrixPosition(x, y)];
        }

        void copyGraph(Graph* graph) {
            int matrixSize = graph->getNodesNum()*(graph->getNodesNum()-1)/2;
            for (int i = 0; i < matrixSize; i++) {
        ...
        void loadGraph(std::string graphName) {
            ...
            // create matrix
            this->matrix = new int[num*(num-1)/2];
            this->nodesNum = num;
...
void addResult(Results* results, Graph* graph, int* cNodes) {
    ...
    newResult->graphWeights = copyVector(graph->getMatrix(), graph->getNodesNum()*(graph->getNodesNum()-1)/2);
